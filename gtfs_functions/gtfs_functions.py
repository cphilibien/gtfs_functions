...
class Feed:
    def __init__(
        ...
        self._stop_freq_by_route = stop_freq_by_route
        self._drop_routes = drop_routes
        self._drop_samedate = drop_samedate
        self._drop_service_after = drop_service_after
        self._dissolve_route = dissolve_route
        self._dissolve_stops = dissolve_stops
    ...
    @property
    def drop_samedate(self):
        return self._drop_samedate

    @property
    def drop_service_after(self):
        return self._drop_service_after

    @property
    def dissolve_route(self):
        return self._dissolve_route

    @property
    def dissolve_stops(self):
        return self._dissolve_stops
    ...
    @property
    def stop_freq_by_route(self):
        return self._stop_freq_by_route
    ...
    def get_routes_patterns(self, trips):
        ...
        trip_stops = trips.merge(self.stop_times, how="left", on="trip_id")
        ...
        trip_stops_zipped = trip_stops.groupby(["trip_id"])["zipped_stops"].apply(list)
        ...
        # trip_stops_zipped_sorted = trip_stops_zipped.apply(str)
        trips_with_stops = trips.merge(trip_stops_zipped_sorted, on="trip_id")
        ...
        trips_with_stops["pattern"] = trips_with_stops.apply(version_hash, axis=1)
        ...
        route_patterns = trips_with_stops.groupby(
            ["route_id", "pattern", "direction_id", "shape_id", "zipped_stops"]
        ).count()[["trip_id"]]
        route_patterns = route_patterns.rename(
            {"trip_id": "cnt_trips"}, axis=1
        ).reset_index()
    ...
    def get_calendar(self):
        calendar = extract_file("calendar", self)

        # suppression des services qui ont lieu une journéé speciale
        if self._drop_samedate:
            calendar = calendar.drop(
                calendar[calendar["start_date"] == calendar["end_date"]].index
            )
        # suppression des services definis pour apres le 20230619 => on ne prend pas en compte les prochains horaires pour ne pas faire des calculs de frequences en double
        if not self._drop_service_after == "":
            # next_date = "20230619"
            next_date = self._drop_service_after
            calendar["start_date"] = calendar["start_date"].astype(str)
            calendar = calendar.drop(
                calendar[calendar["start_date"] >= next_date].index
            )
        ...
        ###### calendar_weekend
        calendar_weekend = calendar[
            (calendar["saturday"] == 1) | (calendar["sunday"] == 1)
        ]
        calendar_saturday = calendar[(calendar["saturday"] == 1)]
        calendar_sunday = calendar[(calendar["sunday"] == 1)]

        match self._daysofweek:
            case "week":
                calendar = calendar_week.copy()
            case "weekend":
                calendar = calendar_weekend.copy()
            case "saturday":
                calendar = calendar_saturday.copy()
            case "sunday":
                calendar = calendar_sunday.copy()
            case _:
                calendar
    ...
    def get_trips(self):
        ...
        if not self._drop_routes == []:
            trips.drop(
                trips[trips.route_id.isin(self._drop_routes)].index, inplace=True
            )
        ...
        ### delete same start and end date, and next services_id
        if not self._daysofweek == "":
            trips = trips[trips["service_id"].isin(self.calendar["service_id"])]
    ...
    def get_routes(self):
        ...
        if not self._drop_routes == []:
            routes.drop(
                routes[routes.route_id.isin(self._drop_routes)].index, inplace=True
            )
    ...
    def get_stop_times(self):
        if self._trips is not None:  # prevents infinite loop
            logging.info("_trips is defined in stop_times in local")
            trips = self._trips
        else:
    ...
    def get_stops_freq(self):
        ...
        if self.stop_freq_by_route:
            stop_frequencies = add_frequency(
                stop_times,
                labels,
                index_=["stop_id", "route_id"],
                col="window",
                cutoffs=cutoffs,
            )
        else:
        ...
        if self._dissolve_stops:
            stop_frequencies = stop_frequencies.dissolve(
                by=["stop_id", "direction_id", "window"],
                aggfunc={
                    "route_id": lambda x: ",".join(str(y) for y in list(x)),
                    "ntrips": "sum",
                },
            ).reset_index()
            start_time = stop_frequencies.window.apply(
                lambda x: cutoffs[labels.index(x)]
            )
            end_time = stop_frequencies.window.apply(
                lambda x: cutoffs[labels.index(x) + 1]
            )

            stop_frequencies["min_per_trip"] = (
                (end_time - start_time) * 60 / stop_frequencies.ntrips
            ).astype(int)
    ...
    def get_lines_freq(self):
        ...
        if self._dissolve_route:
            line_frequencies = line_frequencies.dissolve(
                by=["route_id", "direction_id", "window"], aggfunc="sum"
            ).reset_index()
            start_time = line_frequencies.window.apply(
                lambda x: cutoffs[labels.index(x)]
            )
            end_time = line_frequencies.window.apply(
                lambda x: cutoffs[labels.index(x) + 1]
            )

            line_frequencies["min_per_trip"] = (
                (end_time - start_time) * 60 / line_frequencies.ntrips
            ).astype(int)
